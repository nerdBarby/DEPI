...
## Project Overview
This project involves a comprehensive analysis of breast cancer data to identify patterns and distinguish between Malignant (M) and Benign (B) diagnoses. The analysis utilizes Python for data preprocessing and visualization and concludes with a dashboard summary.
...
* **Boxplots:** Showed clear distinctions between diagnoses. Malignant cases consistently exhibited higher values for `radius_mean`, `perimeter_mean`, `smoothness_mean`, and `concavity_mean` compared to Benign cases.
...
* **Correlation Matrix:** A heatmap was generated to visualize the relationships between features. Strong positive correlations were observed between features such as `radius_mean`, `perimeter_mean`, and `area_mean`.
